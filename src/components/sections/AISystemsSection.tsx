...
// ---------------------------------------------------------------------------
// VideoBg — observes a background wrapper div via a local ref passed through
// Section's style prop; threshold 0, rootMargin 250px so any proximity plays;
// resume after dashboard toggle / visibility change
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// PlatformCard — dispatches dashboard-toggled so VideoBg can re-check play
// ---------------------------------------------------------------------------
...
function PlatformCard({ cover, dashboard, nameEn, nameAr, descEn, descAr }: PlatformCardProps) {
  ...
  function handleToggle() {
    setExpanded((v) => {
      const next = !v
      window.dispatchEvent(new Event('dashboard-toggled'))
      return next
    })
  }
...
export default function AISystemsSection() {
  ...
  const aiCards = [
    { titleEn: 'AI Employees', titleAr: 'موظفون رقميون', bodyEn: 'Trained on your business, answering customers 24/7.', bodyAr: 'يخدمون عملاءك على مدار الساعة بعد تدريبهم على أعمالك.' },
    { titleEn: 'WhatsApp AI Assistants', titleAr: 'مساعدو واتساب الأذكياء', bodyEn: 'Capture, qualify, and book — inside the app Saudis already use.', bodyAr: 'يستقبلون العملاء ويصنّفونهم ويحجزون المواعيد داخل التطبيق الأكثر استخداماً.' },
    { titleEn: 'Sales & Lead Automation', titleAr: 'أتمتة المبيعات', bodyEn: 'Follow-ups that never forget, pipelines that never sleep.', bodyAr: 'تُتابع كل عميل محتمل دون نسيان ودون توقف.' },
    { titleEn: 'Internal Knowledge Chatbots', titleAr: 'روبوتات المعرفة الداخلية', bodyEn: 'Your policies, products, and processes — answerable in seconds.', bodyAr: 'تجيب فريقك عن السياسات والمنتجات والإجراءات في ثوانٍ.' },
    { titleEn: 'AI Content & Commercials', titleAr: 'محتوى وإعلانات بالذكاء الاصطناعي', bodyEn: 'Concept to screen in days, not months.', bodyAr: ' ننقل الفكرة إلى الشاشة في أيام لا أشهر.' },
    { titleEn: 'Custom AI Systems', titleAr: 'أنظمة ذكاء اصطناعي مخصصة', bodyEn: "Built for your workflow when off-the-shelf won't do.", bodyAr: 'نصمّمها خصيصاً لسير عملك عندما لا تكفي الحلول الجاهزة.' },
  ]
  ...
  return (
    ...
    >
      {/* Background: video (IO on bg div) or static image (reduced-motion) */}
      <VideoBg src={AI_BG_LOOP} poster={IMAGES.aiSystemsBg} reducedMotion={reducedMotion} />

      {/* Content layer */}
      <div style={{ position: 'relative', zIndex: 1 }}>
        <p style={{ fontFamily: 'var(--font-body)', fontSize: 'var(--body)', color: 'var(--color-text-dim)', lineHeight: 1.7, maxWidth: '640px', marginBottom: '3rem' }}>
          {isArabic
        ...
        {/* AI cards grid */}
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1.5rem', marginBottom: 'clamp(4rem, 8vw, 8rem)' }}>
          {aiCards.map((card, i) => (
            <div key={i} style={{ background: 'var(--color-card)', border: '1px solid var(--color-card-border)', borderRadius: 'var(--radius)', padding: '1.5rem' }}>
              <h3 style={{ fontFamily: 'var(--font-body)', fontWeight: 600, fontSize: 'var(--h3)', color: 'var(--color-text)', marginBottom: '0.5rem' }}>
                {isArabic ? card.titleAr : card.titleEn}
              </h3>
              <p style={{ fontFamily: 'var(--font-body)', fontSize: 'var(--body-sm)', color: 'var(--color-text-dim)', lineHeight: 1.6 }}>
        ...
        <div style={{ borderTop: '1px solid var(--color-card-border)', paddingTop: 'clamp(3rem, 6vw, 5rem)' }}>
          <p style={{ fontFamily: 'var(--font-body)', fontWeight: 500, fontSize: 'var(--eyebrow)', letterSpacing: '0.2em', textTransform: 'uppercase', color: 'var(--color-gold)', marginBottom: '1rem' }}>
            {isArabic ? 'منصات هندسناها بالكامل' : 'PLATFORMS WE ENGINEERED'}
          </p>
          <p style={{ fontFamily: 'var(--font-body)', fontSize: 'var(--body)', color: 'var(--color-text-dim)', lineHeight: 1.7, maxWidth: '640px', marginBottom: '2.5rem' }}>
            {isArabic
          ...
          <p style={{ fontFamily: 'var(--font-body)', fontSize: 'var(--body-sm)', color: 'var(--color-text-faint)', lineHeight: 1.7, maxWidth: '680px' }}>
            {isArabic
